export default class WebGLRenderer {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    const fs = this.createShader(
      ...
      isGL2
        ? fsSrc
        : `precision mediump float; varying vec2 v_uv; uniform sampler2D u_base; uniform sampler2D u_overlay; void main(){ vec4 b=texture2D(u_base,v_uv); vec4 ovr=texture2D(u_overlay,v_uv); gl_FragColor = ovr.a>0.0 ? (ovr + (1.0-ovr.a)*b) : b; }`,
    );
    const prog = gl.createProgram();
    if (!prog) throw new Error("prog");
  ...
  draw(
    base: HTMLImageElement | ImageBitmap | HTMLCanvasElement,
