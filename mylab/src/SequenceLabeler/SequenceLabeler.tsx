...
const SequenceLabeler: React.FC<{
  ...
  // Dedicated draw throttle refs for coalescing to target FPS
  ...
  /** ===== Drawing (useEffect + FPS throttle, fallback to nearest/last) ===== */
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      ...
      if (!bmp) {
        ...
        for (let d = 1; d <= maxSearch; d++) {
          const before = frame - d;
          const after = frame + d;
          if (before >= 0 && cacheRef.current.has(before)) { bmp = cacheRef.current.get(before)!; drawFrameIndex = before; break; }
          if (after < total && cacheRef.current.has(after)) { bmp = cacheRef.current.get(after)!; drawFrameIndex = after; break; }
        }
        if (!bmp && lastDrawnRef.current.bmp) { bmp = lastDrawnRef.current.bmp; drawFrameIndex = lastDrawnRef.current.frame; }
      }
      ...
      lastDrawnRef.current = { frame: drawFrameIndex, bmp };
      ...
      if (showGhosts && ghostAlpha > 0) {
        for (const t of tracks) {
          ...
          const prev = rectAtFrame(t, drawFrameIndex - 1, interpolate);
          if (prev) drawRect(prev, color, ghostAlpha, true);
          const next = rectAtFrame(t, drawFrameIndex + 1, interpolate);
      ...
      for (const t of tracks) {
        if (t.hidden) continue;
        const r = rectAtFrame(t, drawFrameIndex, interpolate);
        if (!r) continue;
        const color = labelSet.colors[t.class_id] || '#66d9ef';
        const isSel = selectedIds.has(t.track_id);
        drawRect(r, color, isSel ? 1 : 0.7, false);
        ctx.save();
        const cls = labelSet.classes[t.class_id] ?? t.track_id;
      ...
      if (dragRef.current.creating && draftRect) {
        const x = draftRect.x * scale, y = draftRect.y * scale, w = draftRect.w * scale, h = draftRect.h * scale;
        ctx.save();
        ctx.setLineDash([4, 4]);
        ctx.lineWidth = 1.5;
        ctx.strokeStyle = '#fff';
