...
const SequenceLabeler: React.FC<{
  ...
  leftTopExtra,
    ...
    const latestVideoFrameRef = useRef<{ idx: number; bmp: ImageBitmap } | null>(null);
    ...
    useEffect(() => { currentFrameRef.current = frame; });
    ...
    const getImage = useCallback(
      ...
      [meta, files, framesBaseUrl, localFiles, scale, workerActive, requestVideoFrame],
    );
    ...
    useEffect(() => { overlayDirtyRef.current = true; }, [tracks]);
    useEffect(() => { overlayDirtyRef.current = true; }, [selectedIds]);
    useEffect(() => { overlayDirtyRef.current = true; }, [labelSet.classes, labelSet.colors]);
    useEffect(() => { overlayDirtyRef.current = true; }, [interpolate, showGhosts, ghostAlpha]);
    useEffect(() => { overlayDirtyRef.current = true; }, [draftRect]);
    useEffect(() => { overlayDirtyRef.current = true; }, [scale]);
    ...
    useEffect(() => {
      const tick = async (t: number) => {
        ...
        if (!drawBmp) {
          ...
          if (!usingVideo && t - lastDecodeReqRef.current >= decodeIntervalMsRef.current) {
        ...
        if (drawBmp) {
          try {
            ...
            if (String(e?.name || e).includes('InvalidStateError') || String(e).includes('detached')) {
              if (cacheRef.current.has(drawIndex) && cacheRef.current.get(drawIndex) === drawBmp) {
          ...
          if (!usingVideo && !scrubActiveRef.current && t - lastDecodeReqRef.current >= decodeIntervalMsRef.current) {
            for (let d = 1; d <= prefetchRadius; d++) {
              const after = f + d;
              if (after < total && !cacheRef.current.has(after) && !inFlightRef.current.has(after)) {
                lastDecodeReqRef.current = t;
                void getImage(after);
                break;
              }
              const before = f - d;
              if (before >= 0 && !cacheRef.current.has(before) && !inFlightRef.current.has(before)) {
                lastDecodeReqRef.current = t;
          ...
          if (!usingVideo && !scrubActiveRef.current && stride === 1 && drawBmp && drawBmp.width < c.width && !upgradeInFlightRef.current.has(drawIndex)) {
    ...
    /** ===== KF / Presence ops ===== */
    const addKeyframe = useCallback((trackId: string, f: number) => {
    ...
    const deleteKeyframe = useCallback((trackId: string, f: number) => {
      applyTracks(
        (ts) =>
          ts.map((t) => {
