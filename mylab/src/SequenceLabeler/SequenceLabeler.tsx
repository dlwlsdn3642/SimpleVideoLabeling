...
const SequenceLabeler: React.FC<{
    ...
    const tick = async (t: number) => {
      ...
      if (!drawBmp) {
        ...
        if (
          !drawBmp &&
          usingVideo &&
          latestVideoFrameRef.current &&
          latestVideoFrameRef.current.idx === f
        ) {
        ...
        const drawOverlayCtx = (ctx2: CanvasRenderingContext2D) => {
          const drawRect2 = (
            ...
            for (const [dx, dy] of dots)
              ctx2.fillRect(dx - hs, dy - hs, hs * 2, hs * 2);
            ctx2.restore();
          };
          if (ghosts && gAlpha > 0) {
            for (const tck of ts) {
          ...
          for (const tck of ts) {
            ...
            const x = r.x * sc,
              y = r.y * sc,
              w = ctx2.measureText(tag).width + 8;
            ctx2.fillStyle = color;
    ...
    try {
      if (playing)
        vw.postMessage({
          type: "play",
          index: frame,
          fps: targetFPS > 0 ? targetFPS : (meta?.fps ?? 30),
          scale,
        });
  ...
  const { scheduleSeek } = useTimelineSeek({
    setFrame,
    videoWorkerRef,
    currentFrameRef,
    scrubActiveRef,
